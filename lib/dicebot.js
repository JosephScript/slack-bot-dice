...
DiceBot.prototype._welcomeMessage = function () {
    this.postMessageToChannel(this.channels[0].name, 'Shall we play a game?' +
        '\n I can roll the dice. Just say `roll nDX&plusmn;y` to invoke me!',
        {as_user: true}, null);
};
...
DiceBot.prototype._replyWithDice = function (originalMessage) {
    ...
    try {
        ...
        if (matching && matching.length > 0) {
            matching = matching[0].replace(' ', '').toLowerCase();
            ...
            var numberOfDice = matching.split('d')[0].length > 0 && !isNaN(matching.split('d')[0]) ? Number(matching.split('d')[0]) : 1;

            // gets the number to the right of the '+'
            var splitOnPlus = matching.split('+').length > 1 && !isNaN(matching.split('+')[1]) ? Number(matching.split('+')[1].trim()) : 0;

            // gets the number to the right of the '-'
            var splitOnMinus = matching.split('-').length > 1 && !isNaN(matching.split('-')[1]) ? Number(matching.split('-')[1].trim()) : 0;


            // gets the number of sides, and adds the bonus (if any)
            if (splitOnPlus > 0) {
                sides = Number(matching.split('d')[1].split('+')[0]); // number between 'd' and '+'
                value += splitOnPlus;
            } else if (splitOnMinus > 0) {
                sides = Number(matching.split('d')[1].split('-')[0]); // number between 'd' and '-'
                value -= splitOnMinus;
            } else {
                sides = Number(matching.split('d')[1]); // number after 'd'
            }
            ...
            self.postMessageToChannel(channel.name, matching + ': ' + value.toString(), {as_user: true}, null);
        }
    } catch (err) {
        console.log(err);
